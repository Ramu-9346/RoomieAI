...
export function StatusBadge({ status, style }: StatusBadgeProps) {
  ...
  const config: Record<OrderStatus, { label: string; color: string; surface: string }> = {
    ...
    preparing: {
      ...
      surface: colors.warning.surface,
    },
    out_for_delivery: {
      label: 'On the way',
      color: colors.primary.default,
    ...
    delivered: {
      ...
      surface: colors.success.surface,
    },
  ...
  return (
    <View
      style={[styles.baseBadge, { backgroundColor: surface, borderRadius: radius.pill }, style]}
    >
...
export function CountBadge({ count, max = 99, variant = 'error', style }: CountBadgeProps) {
  ...
  const color =
    variant === 'primary' ? colors.primary : variant === 'success' ? colors.success : colors.error;
...
export function RoleBadge({ role, style }: RoleBadgeProps) {
  ...
  return (
    ...
    >
      <Text variant="monoSmall" color={role === 'admin' ? colors.primary.text : colors.text.muted}>
